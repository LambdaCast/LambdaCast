...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for user in orm['auth.user'].objects.filter(user_permissions=add_mediaitem):
            user.user_permissions.add(add_mediafile)
            user.save()
        for group in orm['auth.group'].objects.filter(permissions=add_mediaitem):
            group.permissions.add(add_mediafile)
            group.save()

        change_mediaitem = orm['auth.permission'].objects.get(codename='change_mediaitem')
        change_mediafile = orm['auth.permission'].objects.get(codename='change_mediafile')
        for user in orm['auth.user'].objects.filter(user_permissions=change_mediaitem):
            user.user_permissions.add(change_mediafile)
            user.save()
        for group in orm['auth.group'].objects.filter(permissions=change_mediaitem):
            group.permissions.add(change_mediafile)
            group.save()

        delete_mediaitem = orm['auth.permission'].objects.get(codename='delete_mediaitem')
        delete_mediafile = orm['auth.permission'].objects.get(codename='delete_mediafile')
        for user in orm['auth.user'].objects.filter(user_permissions=delete_mediaitem):
            user.user_permissions.add(delete_mediafile)
            user.save()
        for group in orm['auth.group'].objects.filter(permissions=delete_mediaitem):
            group.permissions.add(delete_mediafile)
            group.save()
